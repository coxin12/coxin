...
#define FTPLIB_DIR_TMSEQ 5
#ifdef  _TMRSEQ
#define FTPLIB_DIR_TMSEQ_R 6
#endif
 ...
 */
#define ftpOpen(x) FtpConnect(x, &DefaultNetbuf, mode)
...
/* !\brief ftp 主动模式/ ftp 被动模式
  ...
  */
GLOBALREF int FtpConnect(const char *host, netbuf **nControl, int mode);
...
GLOBALREF int FtpClose(netbuf *nData, int isErr = 1);
...
GLOBALREF int FtpSize(const char *path, int64_t *size, char mode, netbuf *nControl);
