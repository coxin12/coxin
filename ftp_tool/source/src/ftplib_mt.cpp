...
const char *ftplib_version = "ftplib Release 3.1 6/xx/98, copyright 1996, 1997, 1998 Thomas Pfau";

GLOBALDEF int ftplib_debug = 0;

int n_TimeOut = CONNECT_TIMEOUT;
...
/*
 * print ftplib version
 */
GLOBALREF  void FtpVersion( )
{
    printf("%s\n",ftplib_version);
}
...
GLOBALDEF void FtpSetTimeOut( int seconds )
{
	n_TimeOut = seconds;
}
 ...
 *
 * @Author : devis
 * @Date   : 2004-12-09 ftp 主动模式/ ftp 被动模式
 *
 * GLOBALDEF int FtpConnect(const char *host, netbuf **nControl)
 * return 1 if connected, 0 if not
 *
 */
GLOBALDEF int FtpConnect(const char *host, netbuf **nControl,int mode )
{
    int sControl;
    int on=1;
    netbuf *ctrl;
    char *lhost;
    char *pnum;
	struct addrinfo *readdr;
	struct addrinfo *paddr;
	struct addrinfo arghints;
	...
	memset(&arghints, 0, sizeof(arghints));
	arghints.ai_flags = AI_CANONNAME;
	arghints.ai_family = AF_INET;
	arghints.ai_socktype = SOCK_STREAM;
	arghints.ai_protocol = IPPROTO_TCP;
	...
	if (pnum == NULL)
	{
		if ((retval = getaddrinfo(lhost, "ftp", &arghints, &readdr)) != 0)
		{
	...
	{
		*pnum++ = '\0';
		if ((retval = getaddrinfo(lhost, pnum, &arghints, &readdr)) != 0)
		{
			fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(retval));
            free(lhost);
			return 0;
	...
	while (paddr != NULL)
	{
		if (paddr->ai_protocol == IPPROTO_TCP || paddr->ai_protocol == 0)
			break;
		paddr = paddr->ai_next;
	}
    ...
    if ( (so_flags = fcntl(sControl, F_GETFL, 0)) < 0 )
    {
        // perror("fcntl");
        net_close(sControl);
		freeaddrinfo(readdr);
    ...
    if ( fcntl(sControl, F_SETFL, so_flags | O_NONBLOCK) < 0 )
    {
        // perror("fcntl");
        net_close(sControl);
		freeaddrinfo(readdr);
    ...
    if (connect(sControl, (paddr->ai_addr), paddr->ai_addrlen) == -1)
    {
        if ( errno == EINPROGRESS )
        {
            wtm.tv_sec = n_TimeOut;
            wtm.tv_usec = 0;
            FD_ZERO(&fds);
            FD_SET(sControl, &fds);
            switch ( select(sControl+1, NULL, &fds, NULL, &wtm) )
            {
                case 0:
                ...
                default:
                    errlen = sizeof(int);
                    getsockopt(sControl, SOL_SOCKET, SO_ERROR, &error, (socklen_t*)&errlen);
                    if ( error != 0 )
                    {
                        // perror("connect");
                        net_close(sControl);
    ...
    fcntl( sControl, F_SETFL, so_flags );
...
netbuf **nData)
{
    ...
    switch (typ)
    {
        ...
        case FTPLIB_DIR_VERBOSE:
            ...
            dir = FTPLIB_READ;
            break;
		case FTPLIB_DIR_TMSEQ:
            strcpy(buf,"LIST -rt");
            dir = FTPLIB_READ;
			break;
#ifdef _TMRSEQ
		case FTPLIB_DIR_TMSEQ_R:
            strcpy(buf,"LIST -t");
            dir = FTPLIB_READ;
			break;
#endif
 ...
 */
GLOBALDEF int FtpClose(netbuf *nData, int isErr)
{
    netbuf *ctrl;
    if (nData->dir == FTPLIB_WRITE)
    {
...
/*
 * FtpLsRT - issue a LIST -rt command and write response to output
 *
 * return 1 if successful, 0 otherwise
 */
GLOBALDEF int FtpLsRT(const char *outputfile, const char *path, netbuf *nControl)
{
    return FtpXfer(outputfile, path, nControl, FTPLIB_DIR_TMSEQ, FTPLIB_ASCII);
}

#ifdef _TMRSEQ
GLOBALDEF int FtpLsT(const char *outputfile, const char *path, netbuf *nControl)
{
    return FtpXfer(outputfile, path, nControl, FTPLIB_DIR_TMSEQ_R, FTPLIB_ASCII);
}
#endif
 ...
 * return 1 if successful, 0 otherwise
 */
GLOBALDEF int FtpSize(const char *path, int64_t *size, char mode, netbuf *nControl)
{
    char cmd[256];
    int resp,rv=1;
    int64_t sz;
    ...
    {
        if (sscanf(nControl->response, "%d %lld", &resp, &sz) == 2)
            *size = sz;
        else
            rv = 0;
    }
...
GLOBALDEF void FtpQuit(netbuf *nControl)
{
    ...
    if ( nControl->ctrl != NULL )
    {
        if( (val = fcntl(nControl->ctrl->handle ,F_GETFL,0) ) < 0 )
        {
            // perror("fcntl");
        }
        else
        {
            ...
            if ( accmode == O_RDONLY ||
                ...
                accmode == O_RDWR )
            {
                net_close(nControl->ctrl->handle);
            }
        }

        if( nControl->ctrl->buf != NULL )
        {
            free(nControl->ctrl->buf);
        }
